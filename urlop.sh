...
if ((${#} > 0)); then
    ...
    readonly ocurrences_search_text=$(grep --ignore-case --file "${work_tmp_file}" "${url_list_file}" | \
        dmenu -l 40 -nb "#100" -nf "#b9c0af" -sb "#000" -sf "#afff2f" -i)
else
    readonly ocurrences_search_text=$(cat "${url_list_file}" | dmenu -l 40 -sf "#afff2f" -i)
fi

[[ -z "${ocurrences_search_text}" ]] && exit 0

readonly url_to_open=$(awk '{print $1}' <<< $(echo "${ocurrences_search_text}"))

# Open the filtered URL:
